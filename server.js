...
const portOneConfig = {
  storeId: process.env.PORTONE_STORE_ID || process.env.PORTONE_STORE_KEY || '',
  channelKey: process.env.PORTONE_CHANNEL_KEY || '',
  apiSecret: process.env.PORTONE_API_SECRET || '',
};

const getPortOneIdentityVerification = async (identityVerificationId) => {
  if (!portOneConfig.apiSecret) {
  ...
  const response = await fetch(`https://api.portone.io/identity-verifications/${encodeURIComponent(identityVerificationId)}`, {
    headers: {
      Authorization: `PortOne ${portOneConfig.apiSecret}`,
...
app.get('/api/identity/config', (_req, res) => {
  res.json({
    enabled: Boolean(portOneConfig.storeId && portOneConfig.channelKey && portOneConfig.apiSecret),
...
app.post('/api/identity/verify', async (req, res) => {
  ...
  if (!identityVerificationId) {
    return res.status(400).json({ error: '본인인증 완료 정보가 없습니다.' });
  }
  if (!portOneConfig.apiSecret) {
    return res.status(503).json({ error: '본인인증 서버 설정이 필요합니다.' });
  }
  ...
  try {
    ...
    res.status(500).json({ error: err.message || '본인인증 처리 중 오류가 발생했습니다.' });
  }
});
...
app.get('/api/auth/check-username', async (req, res) => {
  ...
  if (username.length < 3) {
    return res.status(400).json({ error: '아이디는 최소 3자 이상이어야 합니다.' });
  }

  try {
    const result = await pool.query('SELECT username FROM hd_users WHERE username = $1', [username]);
...
app.post('/api/auth/find-id', async (req, res) => {
  ...
  if (!String(name || '').trim() || !normalizedHp) {
    return res.status(400).json({ error: '이름과 핸드폰번호를 입력해 주세요.' });
  }
...
app.post('/api/auth/reset-password', async (req, res) => {
  const { username, name, hp, password } = req.body;
  const normalizedHp = String(hp || '').replace(/\D/g, '');

  if (!String(username || '').trim() || !String(name || '').trim() || !normalizedHp) {
    return res.status(400).json({ error: '아이디, 이름, 핸드폰번호를 입력해 주세요.' });
  }
  if (!password || String(password).length < 3) {
    return res.status(400).json({ error: '새 비밀번호는 3자 이상이어야 합니다.' });
  }
  ...
  try {
    ...
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);
    await pool.query('UPDATE hd_users SET password = $1 WHERE username = $2', [hashedPassword, String(username).trim()]);
